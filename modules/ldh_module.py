...
class LDHModule:
    def __init__(self):
        self.od = pd.read_excel(Path(os.getcwd()) / 'data' / 'ldh' / 'ldh_data.xlsx', sheet_name='OD')
        self.ldh = pd.read_excel(Path(os.getcwd()) / 'data' / 'ldh' / 'ldh_data.xlsx', sheet_name='LDH')
    ...
    def od_adjust(self):
        ...
        self.od['replicate'] = self.od['bio. Rep.'].astype(str) + "_" +  self.od['tech. Rep'].astype(str)
    ...
    def calculate_cytotoxicity(self):
        ...
        mergedf['cytotoxicity'] = (mergedf['Adjusted_OD'] - mergedf['Spontaneous LDH']) / (max_ldh - mergedf['Spontaneous LDH'])
    ...
    def plot_replicates(self, filepath:str=None):
        ...
        for treatment, color in colors.items():
            ...
            plt.scatter(subset['Time (h)'], subset['cytotoxicity'], color=color, label=treatment)
            ...
            for bio_rep in subset['bio. Rep.'].unique():
                for tech_rep in subset['tech. Rep'].unique():
                    ...
                    plt.plot(data_points['Time (h)'], data_points['cytotoxicity'], color=color, linestyle='--', alpha=0.5)
    ...
    def impute_value(self, treatment, time, replicate):
        ...
        subset = self.data[(self.data['Treatment'] == treatment) & (self.data['Time (h)'] == time)]
        ...
        self.data.loc[(self.data['Treatment'] == treatment) & (self.data['Time (h)'] == time) & (self.data['replicate'] == replicate), 'cytotoxicity'] = mean

    def plot_bar_chart(self, filepath:str=None):
            # Group by Treatment and Time (h) and calculate mean and standard error
            grouped = self.data.groupby(['Treatment', 'Time (h)'])['cytotoxicity'].agg(['mean', 'std', 'count'])
            ...
            for idx, (treatment, color) in enumerate(colors.items()):
                ...
                plt.bar([p + idx * width for p in positions], means, yerr=errors, color=color, width=width, label=treatment, capsize=5, align='center')
    ...
    def anova_tukey_analysis(self, output_filename='reports//anova_tukey_results.xlsx'):
        ...
        with pd.ExcelWriter(output_filename, engine='xlsxwriter') as writer:
            ...
            for time in self.data['Time (h)'].unique():
                ...
                if p_value < 0.05:
                    tukey = pairwise_tukeyhsd(endog=subset['cytotoxicity'], groups=subset['Treatment'], alpha=0.05)
